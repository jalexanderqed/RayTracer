...
PolyIntersectionPoint::PolyIntersectionPoint() :
  position(0), poly(NULL) {}

PolyIntersectionPoint::PolyIntersectionPoint(const glm::vec3 &p, const Mesh& py) :
  position(p), poly(py) {}
...
IntersectionPoint::IntersectionPoint(const glm::vec3& p,
				     const GeoObject& o,
...
IntersectionPoint intersectSphere(
  ...
  glm::vec3 oToC = sphere.origin - origin;
  ...
  if (glm::length(oToC) > sphere.radius) distToIntersect = rayProj - halfChordLength;
