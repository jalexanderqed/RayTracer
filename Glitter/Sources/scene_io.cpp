...
read_point_lightA(shared_obj::Scene &scene, FILE *fp) {
    ...
    scene.lights.push_back(shared_obj::Light(pos, color, scene.lights.size(), POINT_LIGHT));
}
...
read_directional_lightA(shared_obj::Scene &scene, FILE *fp) {fscanf(fp, " {");
    ...
    scene.lights.push_back(shared_obj::Light(glm::vec3(0), direction, color, scene.lights.size(), DIRECTIONAL_LIGHT));
}
...
read_spot_lightA(shared_obj::Scene &scene, FILE *fp) {
    ...
    scene.lights.push_back(shared_obj::Light(pos, direction, color, scene.lights.size(), SPOT_LIGHT));
}
