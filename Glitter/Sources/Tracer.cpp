...
glm::vec3 calcAllLights(const IntersectionPoint &iPoint,
  ...
  glm::vec3 diffuse(interMaterial.diffColor[0],
		    interMaterial.diffColor[1],
		    interMaterial.diffColor[2]);
  glm::vec3 specular(interMaterial.specColor[0],
		     interMaterial.specColor[1],
		     interMaterial.specColor[2]);
  glm::vec3 ambient(interMaterial.ambColor[0],
		    interMaterial.ambColor[1],
		    interMaterial.ambColor[2]);
  glm::vec3 emissive(interMaterial.emissColor[0],
		     interMaterial.emissColor[1],
		     interMaterial.emissColor[2]);
  ...
  for (Light l : lights) {
	...
	ipMaterial.ktran > scene_params.EPSILON) {
      glm::vec3 light_contrib = lightContrib(seenColor, normal, inVec, dirToLight,
					     l.light_type() == DIRECTIONAL_LIGHT ? -1 : glm::distance(l.position(), ip.position),
