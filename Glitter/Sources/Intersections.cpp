...
IntersectionPoint intersectScene(
  ...
  if (scene_params.useAcceleration) {
	...
	currPoint = bound->intersect(vec, origin, scene_data, scene_params);
    ...
    for (ObjIO *object = scene_data.sceneGeometry->objects; object != NULL; object = object->next) {
      IntersectionPoint currPoint;
      switch (object->type) {
      case SPHERE_OBJ:
	currPoint = intersectSphere(vec, origin, object, scene_data, scene_params);
	break;
      case POLYSET_OBJ:
	currPoint = intersectPolySet(vec, origin, object, scene_params);
	break;
      default:
